...
function mg_add_secondary_x_axis_label(args, g) {
  if (args.show_secondary_x_label && args.secondary_x_format) {
    var labels = g.selectAll('.mg-xax-labels')
        .data(args.processed.x_ticks).enter()
        .append('text')
        .attr('x', function(d) {
        ...
        .attr('y', ((args.height - args.bottom + args.xax_tick_length * 7 / 3) + 16).toFixed(2))
        .attr('dy', '.50em')
        .attr('text-anchor', 'middle')
        .attr('class', 'secondary');
        labels.text(function(d) {
          return args.xax_units + args.secondary_x_format(d);
        });
  } else if (args.time_series && (args.show_years || args.show_secondary_x_label)) {
    var tf = mg_get_yformat_and_secondary_time_function(args);
...
(function() {
  ...
  function histogram(args) {
    ...
    this.mainPlot = function() {
      ...
      var max_y = d3.max(args.data[0], function(d) { return d[args.y_accessor]; });

      var bar = g.selectAll('.mg-bar')
        .data(args.data[0])
        .enter().append('g')
        .attr('class', function(d) {
          var class_name = "mg-bar";
        ...
        .attr('transform', function(d) {
          if (d[args.y_accessor] < 0) {
            return "translate(" + args.scales.X(d[args.x_accessor]).toFixed(2) + "," + args.scales.Y(max_y).toFixed(2)  + ")";
          } else {
      ...
      bar.append('rect')
        ...
        .attr('height', function(d) {
          ...
          if (d[args.y_accessor] < 0) {
            return (args.height - args.bottom - args.buffer - args.scales.Y(max_y)).toFixed(2);
          }
